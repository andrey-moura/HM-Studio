...
{
	m_pSaveTextButton = new wxButton(this, wxID_ANY, L"Save");
	m_pPrevTextButton = new wxButton(this, wxID_ANY, L"<<");
	m_pNextTextButton = new wxButton(this, wxID_ANY, L">>");

	m_pButtonsSizer = new wxBoxSizer(wxHORIZONTAL);
	m_pButtonsSizer->Add(m_pSaveTextButton);
	m_pButtonsSizer->AddStretchSpacer(1);
	m_pButtonsSizer->Add(m_pPrevTextButton);
	m_pButtonsSizer->AddSpacer(4);
	m_pButtonsSizer->Add(m_pNextTextButton);

	wxSize min_size = m_pPrevTextButton->GetSize();
	min_size.SetWidth(min_size.GetWidth() / 2);
	
	m_pPrevTextButton->SetMinSize(min_size);
	m_pNextTextButton->SetMinSize(min_size);

	m_pSaveTextButton->Bind(wxEVT_BUTTON, &EditorFrame::OnSaveStringClick, this);
	m_pPrevTextButton->Bind(wxEVT_BUTTON, &EditorFrame::OnPrevStringClick, this);
	m_pNextTextButton->Bind(wxEVT_BUTTON, &EditorFrame::OnProxStringClick, this);
}
...
void EditorFrame::CreateMyStatusBar(size_t count)
{
	...
	m_pStatusBar->SetFont(s_StartFont);
}

void EditorFrame::StatusToStc(STC* stc, bool size, bool ln, bool sel, bool col)
{
	stc->SetStatusBar(m_pStatusBar);

	int cur_index = 1;

	if (size)
	{
		stc->SetStatusIndex(cur_index, StcStatus::SIZE);
		++cur_index;
	}
	if (ln)
	{
		stc->SetStatusIndex(cur_index, StcStatus::LINE);
		++cur_index;
	}
	if (sel)
	{
		stc->SetStatusIndex(cur_index, StcStatus::SELECTION);
		++cur_index;
	}
	if (col)
	{
